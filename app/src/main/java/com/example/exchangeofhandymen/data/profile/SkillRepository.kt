...
class SkillRepository @Inject constructor() {
    ...
    suspend fun getAllSkills(): ArrayList<Skill> {
        ...
        allSkills.documents.forEach {
            listSkills.add(Skill(it.id,it.get("Name") as String,it.get("Population").toString().toInt()))
        }
        return listSkills
    }
    ...
    suspend fun addNewSkill(skill: Skill):String{
        ...
        dbFirestoreProfile.collection("User").document(auth.uid!!).update("skills",newSkills).await()
        return idSkill
    }
    ...
    suspend fun addOldSkill(skillId:String){
        ...
        val oldPopulation = dbFirestoreProfile.collection("Skill").document(skillId).get().await().get("Population").toString().toInt()

        dbFirestoreProfile.collection("Skill").document(skillId).update("Population",oldPopulation+1).await()
